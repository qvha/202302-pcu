...
def main():
  ...
  print( "━"*35+" IA_CORE domain "+"━"*55 )
  domain=IA_CORE
  mailbox_OC_CAPABILITY(domain)
  mailbox_READ_FUSED_P0_RATIO(domain,0 )
  mailbox_READ_VF_OVERRIDE(domain)

  print( "━"*34+" LLC & RING domain "+"━"*53 )
  domain=LLC_AND_RING
  mailbox_OC_CAPABILITY(domain)
  mailbox_READ_FUSED_P0_RATIO(domain, 0)
  mailbox_READ_VF_OVERRIDE(domain)
  ...
  print( "━"*106 )
  mailbox_READ_BCLK_FREQUENCY()
  mailbox_READ_OC_STATUS()

  for limits in (False, True):
    for avx_level in (0,1,2,3):  
      mailbox_PER_CORE_RATIO_LIMITS_CAP(0, limits, avx_level)

  msr.read_PLATFORM_POWER_LIMIT_SRVR()
  msr.read_VR_CURRENT_CONFIG()
  msr.read_FLEX_RATIO(0)
#  msr.read_FLEX_RATIO_CFG(238)

  mailbox_READ_MISC_GLOBAL_CONF()
  mailbox_READ_AVX_CONTROL()

#  for i in range(NCPU):
#    mailbox_READ_FUSED_P0_RATIO(IA_CORE, i)

  print( "━"*15, "iMVP8 is driving SVID control on 3 rails : core, GT, SA", "━"*33)
  for i in range(4):
    mailbox_READ_SVID_CONFIG(i)

  print( "━"*15, "7.14 AVX Negative ratio offset", "━"*33)
  msr.read_TURBO_RATIO_LIMIT(core=0)
  msr.read_PLATFORM_INFO(core=0)

  print( "━"*15, "7.3 Configurable TDP", "━"*50)
  print("""To support runtime power limits and MAX Turbo Ratio override, Software or Firmware are able to runtime change :
 - the Turbo Ratio Limit through TURBO_RATIO_LIMIT MSR 1ADh and OC Mailbox 150h,
 - Package Power Limits through PKG_POWER_LIMIT MSR 610h and Package Current Limits through MSR 601h[12:0]""")

  print( msr.read_PACKAGE_RAPL_LIMIT_CFG() )

  print( "attempting to change the limits" )
  [ msr.write_PACKAGE_RAPL_LIMIT_CFG(3, 31, 500*8, 3, 31, 764*8, i) for i in range(NCPU) ]
  print( msr.read_PACKAGE_RAPL_LIMIT_CFG(core=NCPU-1) )

  turbo_mode, tpr_message, limit_cpuid_maxval, monitor_fsm, enhanced_speedstep,\
    pebs,bts,perfmon, tcc, fast_strings = msr.read_IA32_MISC_ENABLE()

  for core in range(NCPU):
    msr.write_IA32_MISC_ENABLE(turbo_mode, tpr_message, limit_cpuid_maxval, monitor_fsm, enhanced_speedstep,\
                           pebs,bts,perfmon, False, fast_strings, core)
  msr.read_IA32_MISC_ENABLE(core=NCPU-1)

#  msr.read_PACKAGE_THERMAL_MARGIN()

#  msr.read_TURBO_ACTIVATION_RATIO(0)
# the following fails 
#  msr.write_TURBO_ACTIVATION_RATIO(25, 0)
#  msr.read_TURBO_ACTIVATION_RATIO(0)

  print( msr.read_PACKAGE_POWER_SKU(0) )
  print( msr.read_VR_CURRENT_CONFIG(0) )
  [ msr.write_VR_CURRENT_CONFIG(900, core) for core in range(NCPU) ]  # current_limit = 900A
  print( msr.read_VR_CURRENT_CONFIG(core=NCPU-1) )
